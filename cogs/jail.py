...
from cogs.helpers import commandhelp, noperms, blacklist, sendmsg
...
class jail(commands.Cog):
    ...
    async def jail(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      try:
       ...
       success = discord.Embed(color=0x495063, description=f"{Emojis.check} {member.mention} got jailed - {reason}")
       await sendmsg(self, ctx, None, success, None, None, None) 
      except Exception as e:
       embed = discord.Embed(color=0x495063, description=f"{ctx.author.mention} there was a problem jailing {member.mention}")
       await sendmsg(self, ctx, None, embed, None, None, None) 
    ...
    async def unjail(self, ctx, *, member: discord.Member=None):
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      await self.bot.db.commit()
      embed = discord.Embed(color=0x495063, description=f"{Emojis.check} {ctx.author.mention}: {member.mention} unjailed")             
      await sendmsg(self, ctx, None, embed, None, None, None) 
